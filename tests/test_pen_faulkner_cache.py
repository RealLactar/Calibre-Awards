"""Offline coverage for PEN/Faulkner keyed landing and year cache."""

from __future__ import annotations

import hashlib
import json
import unittest
from datetime import datetime, timezone
from pathlib import Path
from tempfile import TemporaryDirectory
from unittest.mock import patch

from awards import cache
from awards.cache_control import refresh_award_source_cache
from awards.sources import hugo, pen_faulkner as pf
from tests.test_pen_faulkner_parser import (
    _PAIRS_2025,
    _PAIRS_2026,
    _finalists_article,
    _finalists_url,
    _historical_landing,
    _landing_url,
    _winner_article,
    _winner_url,
)

_UTC = timezone.utc
_STALE_AT = datetime(2020, 1, 1, tzinfo=_UTC)


def _entry_path(cache_dir: Path, entry_kind: str, entry_key: str) -> Path:
    digest = hashlib.sha256(entry_key.encode('utf-8')).hexdigest()
    return cache_dir / pf.SOURCE_KEY / entry_kind / f'{digest}.json'


class _HttpTracker:
    def __init__(self, pages):
        self.pages = pages
        self.calls = []

    def fetch_response(self, url: str):
        self.calls.append(url)
        body = self.pages.get(url)
        if body == 'FAIL':
            raise pf.PenFaulknerSourceError(f'HTTP failed for {url}')
        if body == '404' or body is None:
            return 404, '', url
        if isinstance(body, tuple):
            return body
        return 200, body, url


def _save_landing(snapshot, *, generated_at=None, ttl_seconds=None, version=None):
    cache.save_cache_entry(
        pf.SOURCE_KEY,
        pf.ARCHIVE_ENTRY_KIND,
        pf.ARCHIVE_ENTRY_KEY,
        pf.ARCHIVE_CACHE_VERSION if version is None else version,
        records=[pf._record_to_cache_dict(record) for record in snapshot.records],
        source_urls=[pf.SOURCE_HOME_URL],
        coverage=pf._archive_coverage(),
        ttl_seconds=(
            pf.HISTORICAL_CACHE_TTL_SECONDS if ttl_seconds is None else ttl_seconds
        ),
        generated_at=generated_at,
    )


def _save_year(snapshot, *, generated_at=None, ttl_seconds=None, version=None):
    cache.save_cache_entry(
        pf.SOURCE_KEY,
        pf.YEAR_ENTRY_KIND,
        pf._year_entry_key(snapshot.award_year),
        pf.YEAR_CACHE_VERSION if version is None else version,
        records=[pf._record_to_cache_dict(record) for record in snapshot.records],
        source_urls=list(snapshot.source_urls),
        coverage=pf._year_coverage(snapshot.award_year, snapshot.state),
        ttl_seconds=(
            pf._year_ttl_seconds(snapshot.state)
            if ttl_seconds is None
            else ttl_seconds
        ),
        generated_at=generated_at,
    )


def _landing_snapshot():
    html = _historical_landing()
    records = pf._parse_landing_html(html, _landing_url())
    pf._validate_historical_records(records)
    return pf._ArchiveSnapshot(records=records, source_url=_landing_url())


def _completed_year(year: int, pairs, winner_title: str, winner_author: str):
    others = [
        (title, author)
        for title, author, _publisher in pairs
        if title != winner_title
    ]
    finalists_html = _finalists_article(
        year,
        pairs,
        style='colon_lines' if year == 2026 else 'prose',
    )
    winner_html = _winner_article(year, winner_title, winner_author, others)
    finalist_records = pf._parse_finalists_html(
        finalists_html, year, _finalists_url(year)
    )
    winner = pf._parse_winner_html(winner_html, year, _winner_url(year))
    merged = pf._dedupe_records(list(finalist_records) + [winner])
    return pf._YearSnapshot(
        award_year=year,
        state='winner',
        source_urls=(_winner_url(year), _finalists_url(year)),
        records=merged,
    ), {_finalists_url(year): finalists_html, _winner_url(year): winner_html}


def _finalist_only_year(year: int, pairs):
    html = _finalists_article(year, pairs, style='prose')
    records = pf._parse_finalists_html(html, year, _finalists_url(year))
    return pf._YearSnapshot(
        award_year=year,
        state='finalist',
        source_urls=(_finalists_url(year),),
        records=records,
    ), {_finalists_url(year): html}


def _absent_year(year: int) -> pf._YearSnapshot:
    return pf._YearSnapshot(
        award_year=year,
        state='absent',
        source_urls=(),
        records=(),
    )


def _seed_unused_modern_years(live_years, utc_year: int) -> None:
    live = set(live_years)
    end = max(pf.MAX_VERIFIED_YEAR, utc_year)
    for year in range(pf.HISTORICAL_ARCHIVE_MAX_YEAR + 1, end + 1):
        if year in live:
            continue
        existing = cache.load_cache_entry(
            pf.SOURCE_KEY,
            pf.YEAR_ENTRY_KIND,
            pf._year_entry_key(year),
            pf.YEAR_CACHE_VERSION,
        )
        if existing is not None:
            continue
        _save_year(_absent_year(year))


def _live_years_from_pages(pages, utc_year: int) -> set[int]:
    live: set[int] = set()
    for year, urls in pf.VERIFIED_YEAR_URLS.items():
        if urls['winner'] in pages or urls['finalists'] in pages:
            live.add(year)
    for url in pages:
        path_year = pf._path_year(url)
        if path_year is not None and path_year > pf.HISTORICAL_ARCHIVE_MAX_YEAR:
            live.add(path_year)
    if pf.AWARD_NEWS_REST_URL in pages and utc_year > pf.MAX_VERIFIED_YEAR:
        live.add(utc_year)
    return live


class PenFaulknerCacheTests(unittest.TestCase):
    def setUp(self):
        self._temp = TemporaryDirectory()
        self.cache_dir = Path(self._temp.name)
        cache.set_cache_directory(self.cache_dir)
        pf._reset_runtime_state()
        hugo._reset_runtime_state()

    def tearDown(self):
        pf._reset_runtime_state()
        hugo._reset_runtime_state()
        cache.set_cache_directory(None)
        self._temp.cleanup()

    def _lookup(self, pages, title, author, utc_year=2026, live_years=None):
        if live_years is None:
            live_years = _live_years_from_pages(pages, utc_year)
        _seed_unused_modern_years(live_years, utc_year)
        tracker = _HttpTracker(pages)
        with patch.object(pf, '_fetch_response', tracker.fetch_response), patch.object(
            pf, '_current_calendar_year', return_value=utc_year
        ):
            results = pf.lookup(title, author)
        return results, tracker

    def test_cold_landing_is_one_get(self):
        pages = {_landing_url(): _historical_landing()}
        results, tracker = self._lookup(
            pages, 'How German Is It?', 'Walter Abish', utc_year=2018
        )
        self.assertEqual(tracker.calls, [_landing_url()])
        self.assertEqual(results[0].status, 'Winner')
        self.assertEqual(results[0].award_year, 1981)
        payload = cache.load_cache_entry(
            pf.SOURCE_KEY,
            pf.ARCHIVE_ENTRY_KIND,
            pf.ARCHIVE_ENTRY_KEY,
            pf.ARCHIVE_CACHE_VERSION,
        )
        self.assertNotIn('html', payload)
        self.assertTrue(all('html' not in item for item in payload['records']))

    def test_fresh_landing_is_zero_http(self):
        _save_landing(_landing_snapshot())
        results, tracker = self._lookup(
            {}, 'The Transit of Venus', 'Shirley Hazzard', utc_year=2018
        )
        self.assertEqual(tracker.calls, [])
        self.assertEqual(results[0].status, 'Finalist')

    def test_ram_reset_fresh_disk_is_zero_http(self):
        _save_landing(_landing_snapshot())
        pf._reset_runtime_state()
        results, tracker = self._lookup(
            {}, 'Behold the Dreamers', 'Imbolo Mbue', utc_year=2018
        )
        self.assertEqual(tracker.calls, [])
        self.assertEqual(results[0].source_url, _landing_url())

    def test_stale_landing_slot_won_refreshes_once(self):
        _save_landing(_landing_snapshot(), generated_at=_STALE_AT, ttl_seconds=60)
        pages = {_landing_url(): _historical_landing()}
        with cache.lookup_refresh_budget():
            results, tracker = self._lookup(
                pages, 'Waiting', 'Ha Jin', utc_year=2018
            )
        self.assertEqual(tracker.calls, [_landing_url()])
        self.assertEqual(results[0].status, 'Winner')

    def test_stale_landing_slot_denied_uses_stale(self):
        _save_landing(_landing_snapshot(), generated_at=_STALE_AT, ttl_seconds=60)
        with cache.lookup_refresh_budget():
            self.assertTrue(cache.try_claim_stale_refresh())
            results, tracker = self._lookup(
                {_landing_url(): 'FAIL'},
                'Improvement',
                'Joan Silber',
                utc_year=2018,
            )
        self.assertEqual(tracker.calls, [])
        self.assertEqual(results[0].status, 'Winner')

    def test_stale_refresh_failure_keeps_stale(self):
        snapshot = _landing_snapshot()
        _save_landing(snapshot, generated_at=_STALE_AT, ttl_seconds=60)
        with cache.lookup_refresh_budget():
            results, tracker = self._lookup(
                {_landing_url(): 'FAIL'},
                'Delicious Foods',
                'James Hannaham',
                utc_year=2018,
            )
        self.assertEqual(tracker.calls, [_landing_url()])
        self.assertEqual(results[0].work_author, 'James Hannaham')
        loaded = cache.load_cache_entry(
            pf.SOURCE_KEY,
            pf.ARCHIVE_ENTRY_KIND,
            pf.ARCHIVE_ENTRY_KEY,
            pf.ARCHIVE_CACHE_VERSION,
        )
        self.assertEqual(loaded['generated_at'], _STALE_AT.strftime('%Y-%m-%dT%H:%M:%SZ'))

    def test_missing_landing_fetches_live(self):
        pages = {_landing_url(): _historical_landing()}
        _results, tracker = self._lookup(
            pages, 'Billy Bathgate', 'E.L. Doctorow', utc_year=2018
        )
        self.assertEqual(tracker.calls, [_landing_url()])

    def test_malformed_and_version_mismatch_fetch_live(self):
        path = _entry_path(self.cache_dir, pf.ARCHIVE_ENTRY_KIND, pf.ARCHIVE_ENTRY_KEY)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text('{not json', encoding='utf-8')
        pages = {_landing_url(): _historical_landing()}
        _results, tracker = self._lookup(
            pages, 'Dusk', 'James Salter', utc_year=2018
        )
        self.assertEqual(tracker.calls, [_landing_url()])
        pf._reset_runtime_state()
        _save_landing(_landing_snapshot(), version=99)
        _results, tracker = self._lookup(
            pages, 'Dusk', 'James Salter', utc_year=2018
        )
        self.assertEqual(tracker.calls, [_landing_url()])

    def test_failed_historical_validation_does_not_save(self):
        pages = {_landing_url(): _historical_landing(skip_year=1990)}
        with self.assertRaises(pf.PenFaulknerSourceError):
            self._lookup(pages, 'Affliction', 'Russell Banks', utc_year=2018)
        self.assertIsNone(
            cache.load_cache_entry(
                pf.SOURCE_KEY,
                pf.ARCHIVE_ENTRY_KIND,
                pf.ARCHIVE_ENTRY_KEY,
                pf.ARCHIVE_CACHE_VERSION,
            )
        )

    def test_cold_completed_year_fetches_winner_and_finalists(self):
        landing = _landing_snapshot()
        _save_landing(landing)
        snapshot, pages = _completed_year(
            2026, _PAIRS_2026, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        results, tracker = self._lookup(
            pages, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        self.assertEqual(set(tracker.calls), set(pages))
        self.assertEqual(len(tracker.calls), 2)
        self.assertEqual(results[0].status, 'Winner')
        self.assertEqual(results[0].award_year, 2026)
        self.assertEqual(results[0].source_url, _winner_url(2026))
        payload = cache.load_cache_entry(
            pf.SOURCE_KEY,
            pf.YEAR_ENTRY_KIND,
            '2026',
            pf.YEAR_CACHE_VERSION,
        )
        self.assertEqual(payload['coverage']['state'], 'winner')
        self.assertNotIn('html', payload)
        self.assertTrue(all('content' not in item for item in payload['records']))

    def test_fresh_completed_year_is_zero_http(self):
        _save_landing(_landing_snapshot())
        snapshot, _pages = _completed_year(
            2025, _PAIRS_2025, 'Small Rain', 'Garth Greenwell'
        )
        _save_year(snapshot)
        results, tracker = self._lookup({}, 'Ghostroots', '’Pemi Aguda')
        self.assertEqual(tracker.calls, [])
        self.assertEqual(results[0].status, 'Finalist')

    def test_stale_completed_year_slot_won_refreshes_only_that_year(self):
        _save_landing(_landing_snapshot())
        snap_2025, pages_2025 = _completed_year(
            2025, _PAIRS_2025, 'Small Rain', 'Garth Greenwell'
        )
        snap_2026, pages_2026 = _completed_year(
            2026, _PAIRS_2026, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        _save_year(snap_2025, generated_at=_STALE_AT, ttl_seconds=60)
        _save_year(snap_2026)
        pages = {**pages_2025, **pages_2026}
        with cache.lookup_refresh_budget():
            _results, tracker = self._lookup(pages, 'Small Rain', 'Garth Greenwell')
        self.assertTrue(set(tracker.calls) <= set(pages_2025))
        self.assertTrue(tracker.calls)
        self.assertNotIn(_winner_url(2026), tracker.calls)

    def test_stale_completed_slot_denied_uses_stale(self):
        _save_landing(_landing_snapshot())
        snapshot, pages = _completed_year(
            2026, _PAIRS_2026, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        _save_year(snapshot, generated_at=_STALE_AT, ttl_seconds=60)
        with cache.lookup_refresh_budget():
            self.assertTrue(cache.try_claim_stale_refresh())
            results, tracker = self._lookup(
                {url: 'FAIL' for url in pages},
                'The White Hot',
                'Quiara Alegría Hudes',
            )
        self.assertEqual(tracker.calls, [])
        self.assertEqual(results[0].status, 'Finalist')

    def test_stale_year_refresh_failure_keeps_stale(self):
        _save_landing(_landing_snapshot())
        snapshot, pages = _completed_year(
            2026, _PAIRS_2026, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        _save_year(snapshot, generated_at=_STALE_AT, ttl_seconds=60)
        with cache.lookup_refresh_budget():
            results, tracker = self._lookup(
                {url: 'FAIL' for url in pages},
                'Dominion',
                'Addie E. Citchens',
            )
        self.assertTrue(tracker.calls)
        self.assertEqual(results[0].work_title, 'Dominion')
        loaded = cache.load_cache_entry(
            pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2026', pf.YEAR_CACHE_VERSION
        )
        self.assertEqual(loaded['generated_at'], _STALE_AT.strftime('%Y-%m-%dT%H:%M:%SZ'))

    def test_winner_absent_finalists_use_short_ttl(self):
        _save_landing(_landing_snapshot())
        snapshot, pages = _finalist_only_year(2026, _PAIRS_2026)
        results, tracker = self._lookup(pages, 'Dominion', 'Addie E. Citchens')
        self.assertEqual(results[0].status, 'Finalist')
        payload = cache.load_cache_entry(
            pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2026', pf.YEAR_CACHE_VERSION
        )
        self.assertEqual(payload['coverage']['state'], 'finalist')
        self.assertEqual(payload['ttl_seconds'], pf.CURRENT_CACHE_TTL_SECONDS)

    def test_winner_later_becomes_completed(self):
        _save_landing(_landing_snapshot())
        finalist_snapshot, finalist_pages = _finalist_only_year(2026, _PAIRS_2026)
        _save_year(finalist_snapshot, generated_at=_STALE_AT, ttl_seconds=60)
        completed, pages = _completed_year(
            2026, _PAIRS_2026, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        with cache.lookup_refresh_budget():
            results, _tracker = self._lookup(
                pages, 'Small Scale Sinners', 'Mahreen Sohail'
            )
        self.assertEqual(results[0].status, 'Winner')
        payload = cache.load_cache_entry(
            pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2026', pf.YEAR_CACHE_VERSION
        )
        self.assertEqual(payload['coverage']['state'], 'winner')
        self.assertEqual(payload['ttl_seconds'], pf.HISTORICAL_CACHE_TTL_SECONDS)

    def test_future_absent_uses_short_ttl(self):
        _save_landing(_landing_snapshot())
        rest = json.dumps([])
        pages = {
            pf.AWARD_NEWS_REST_URL: rest,
        }
        results, tracker = self._lookup(
            pages, 'No Such Book', 'No Such Author', utc_year=2027
        )
        self.assertEqual(results, [])
        self.assertIn(pf.AWARD_NEWS_REST_URL, tracker.calls)
        payload = cache.load_cache_entry(
            pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2027', pf.YEAR_CACHE_VERSION
        )
        self.assertEqual(payload['coverage']['state'], 'absent')
        self.assertEqual(payload['ttl_seconds'], pf.CURRENT_CACHE_TTL_SECONDS)
        self.assertEqual(payload['records'], [])

    def test_fresh_absent_is_zero_discovery_http(self):
        _save_landing(_landing_snapshot())
        _save_year(
            pf._YearSnapshot(
                award_year=2027,
                state='absent',
                source_urls=(),
                records=(),
            )
        )
        results, tracker = self._lookup(
            {pf.AWARD_NEWS_REST_URL: 'FAIL'},
            'No Such Book',
            'No Such Author',
            utc_year=2027,
        )
        self.assertEqual(results, [])
        self.assertEqual(tracker.calls, [])

    def test_modern_years_are_independent(self):
        _save_landing(_landing_snapshot())
        snap_2025, pages_2025 = _completed_year(
            2025, _PAIRS_2025, 'Small Rain', 'Garth Greenwell'
        )
        _save_year(snap_2025)
        snap_2026, pages_2026 = _completed_year(
            2026, _PAIRS_2026, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        pages = {**pages_2026, **{url: 'FAIL' for url in pages_2025}}
        results, tracker = self._lookup(pages, 'Small Rain', 'Garth Greenwell')
        self.assertEqual(results[0].status, 'Winner')
        self.assertTrue(set(tracker.calls).isdisjoint(pages_2025))

    def test_failed_year_does_not_delete_sibling(self):
        _save_landing(_landing_snapshot())
        snap_2025, _pages_2025 = _completed_year(
            2025, _PAIRS_2025, 'Small Rain', 'Garth Greenwell'
        )
        _save_year(snap_2025)
        pages = {
            _winner_url(2026): 'FAIL',
            _finalists_url(2026): 'FAIL',
        }
        results, _tracker = self._lookup(pages, 'Small Rain', 'Garth Greenwell')
        self.assertEqual(results[0].award_year, 2025)
        sibling = cache.load_cache_entry(
            pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2025', pf.YEAR_CACHE_VERSION
        )
        self.assertIsNotNone(sibling)
        self.assertIsNone(
            cache.load_cache_entry(
                pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2026', pf.YEAR_CACHE_VERSION
            )
        )

    def test_longlist_is_not_persisted(self):
        _save_landing(_landing_snapshot())
        snapshot, pages = _completed_year(
            2026, _PAIRS_2026, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        self._lookup(pages, 'King of Ashes', 'S.A. Cosby')
        payload = cache.load_cache_entry(
            pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2026', pf.YEAR_CACHE_VERSION
        )
        titles = [item['work_title'] for item in payload['records']]
        self.assertNotIn('King of Ashes', titles)

    def test_manual_refresh_clears_archive_years_and_ram_with_zero_http(self):
        _save_landing(_landing_snapshot())
        snapshot, pages = _completed_year(
            2026, _PAIRS_2026, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        _save_year(snapshot)
        pf._store_landing_snapshot(_landing_snapshot())
        pf._store_year_snapshot(snapshot)
        hugo._archive_records_cache = ()
        cache.save_source_cache(
            'hugo',
            1,
            records=[{'title': 'hugo', 'year': 2020}],
            source_urls=['https://example.test/hugo'],
            coverage={'source': 'hugo'},
            ttl_seconds=3600,
        )
        tracker = _HttpTracker(pages)
        with patch.object(pf, '_fetch_response', tracker.fetch_response):
            self.assertTrue(refresh_award_source_cache('pen_faulkner'))
        self.assertEqual(tracker.calls, [])
        self.assertIsNone(pf._ram_landing())
        self.assertIsNone(pf._ram_year(2026))
        self.assertIsNone(
            cache.load_cache_entry(
                pf.SOURCE_KEY,
                pf.ARCHIVE_ENTRY_KIND,
                pf.ARCHIVE_ENTRY_KEY,
                pf.ARCHIVE_CACHE_VERSION,
            )
        )
        self.assertIsNone(
            cache.load_cache_entry(
                pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2026', pf.YEAR_CACHE_VERSION
            )
        )
        self.assertTrue((self.cache_dir / 'hugo.json').is_file())
        self.assertEqual(hugo._archive_records_cache, ())

    def test_discovery_then_finalist_then_winner(self):
        _save_landing(_landing_snapshot())
        rest_finalists = json.dumps(
            [
                {
                    'title': {
                        'rendered': 'Announcing the Finalists for the 2027 PEN/Faulkner Award for Fiction'
                    },
                    'slug': 'announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction',
                    'link': 'https://www.penfaulkner.org/2027/03/02/announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction/',
                }
            ]
        )
        pairs = _PAIRS_2026
        finalists_html = _finalists_article(2027, pairs, style='prose')
        pages = {
            pf.AWARD_NEWS_REST_URL: rest_finalists,
            'https://www.penfaulkner.org/2027/03/02/announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction/': finalists_html,
        }
        results, tracker = self._lookup(
            pages, 'Dominion', 'Addie E. Citchens', utc_year=2027
        )
        self.assertEqual(results[0].status, 'Finalist')
        self.assertIn(pf.AWARD_NEWS_REST_URL, tracker.calls)
        payload = cache.load_cache_entry(
            pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2027', pf.YEAR_CACHE_VERSION
        )
        self.assertEqual(payload['coverage']['state'], 'finalist')
        self.assertNotIn('html', payload)

        pf._reset_runtime_state()
        rest_both = json.dumps(
            [
                {
                    'title': {
                        'rendered': 'Announcing the Finalists for the 2027 PEN/Faulkner Award for Fiction'
                    },
                    'slug': 'announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction',
                    'link': 'https://www.penfaulkner.org/2027/03/02/announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction/',
                },
                {
                    'title': {
                        'rendered': 'Announcing the Winner of the 2027 PEN/Faulkner Award for Fiction'
                    },
                    'slug': 'announcing-the-winner-of-the-2027-pen-faulkner-award-for-fiction',
                    'link': 'https://www.penfaulkner.org/2027/04/06/announcing-the-winner-of-the-2027-pen-faulkner-award-for-fiction/',
                },
            ]
        )
        winner_html = _winner_article(
            2027, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        pages = {
            pf.AWARD_NEWS_REST_URL: rest_both,
            'https://www.penfaulkner.org/2027/03/02/announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction/': finalists_html,
            'https://www.penfaulkner.org/2027/04/06/announcing-the-winner-of-the-2027-pen-faulkner-award-for-fiction/': winner_html,
        }
        _save_year(
            pf._YearSnapshot(
                award_year=2027,
                state='finalist',
                source_urls=(
                    'https://www.penfaulkner.org/2027/03/02/announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction/',
                ),
                records=pf._parse_finalists_html(
                    finalists_html,
                    2027,
                    'https://www.penfaulkner.org/2027/03/02/announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction/',
                ),
            ),
            generated_at=_STALE_AT,
            ttl_seconds=60,
        )
        with cache.lookup_refresh_budget():
            results, _tracker = self._lookup(
                pages, 'Small Scale Sinners', 'Mahreen Sohail', utc_year=2027
            )
        self.assertEqual(results[0].status, 'Winner')
        payload = cache.load_cache_entry(
            pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2027', pf.YEAR_CACHE_VERSION
        )
        self.assertEqual(payload['coverage']['state'], 'winner')

    def test_malformed_year_fetches_live(self):
        _save_landing(_landing_snapshot())
        path = _entry_path(self.cache_dir, pf.YEAR_ENTRY_KIND, '2026')
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text('{not json', encoding='utf-8')
        snapshot, pages = _completed_year(
            2026, _PAIRS_2026, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        results, tracker = self._lookup(
            pages, 'Small Scale Sinners', 'Mahreen Sohail', live_years={2026}
        )
        self.assertTrue(tracker.calls)
        self.assertEqual(results[0].status, 'Winner')

    def test_year_version_mismatch_fetches_live(self):
        _save_landing(_landing_snapshot())
        snapshot, pages = _completed_year(
            2026, _PAIRS_2026, 'Small Scale Sinners', 'Mahreen Sohail'
        )
        _save_year(snapshot, version=99)
        results, tracker = self._lookup(
            pages, 'Heart the Lover', 'Lily King', live_years={2026}
        )
        self.assertTrue(tracker.calls)
        self.assertEqual(results[0].status, 'Finalist')

    def test_2021_identical_urls_fetch_once(self):
        _save_landing(_landing_snapshot())
        html = _winner_article(
            2021,
            'The Secret Lives of Church Ladies',
            'Deesha Philyaw',
            [
                ('Disappear Doppelgänger Disappear', 'Matthew Salesses'),
                ('The Knockout Queen', 'Rufi Thorpe'),
                ('Mother Daughter Widow Wife', 'Robin Wasserman'),
                ('Scattered Lights', 'Steve Wiegenstein'),
            ],
        )
        url = _winner_url(2021)
        self.assertEqual(url, _finalists_url(2021))
        results, tracker = self._lookup(
            {url: html},
            'The Secret Lives of Church Ladies',
            'Deesha Philyaw',
            live_years={2021},
        )
        self.assertEqual(tracker.calls, [url])
        self.assertEqual(results[0].status, 'Winner')
        payload = cache.load_cache_entry(
            pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2021', pf.YEAR_CACHE_VERSION
        )
        self.assertEqual(payload['coverage']['state'], 'winner')
        self.assertEqual(len(payload['records']), 5)

    def test_ambiguous_discovery_fails_closed(self):
        _save_landing(_landing_snapshot())
        rest = json.dumps(
            [
                {
                    'title': {
                        'rendered': 'Announcing the Finalists for the 2027 PEN/Faulkner Award for Fiction'
                    },
                    'slug': 'announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction',
                    'link': 'https://www.penfaulkner.org/2027/03/02/announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction/',
                },
                {
                    'title': {
                        'rendered': 'Announcing the Finalists for the 2027 PEN/Faulkner Award for Fiction'
                    },
                    'slug': 'announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction-2',
                    'link': 'https://www.penfaulkner.org/2027/03/03/announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction-2/',
                },
            ]
        )
        results, _tracker = self._lookup(
            {pf.AWARD_NEWS_REST_URL: rest},
            'Dominion',
            'Addie E. Citchens',
            utc_year=2027,
        )
        self.assertEqual(results, [])
        self.assertIsNone(
            cache.load_cache_entry(
                pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2027', pf.YEAR_CACHE_VERSION
            )
        )

    def test_url_body_year_disagreement_does_not_cache(self):
        _save_landing(_landing_snapshot())
        rest = json.dumps(
            [
                {
                    'title': {
                        'rendered': 'Announcing the Winner of the 2027 PEN/Faulkner Award for Fiction'
                    },
                    'slug': 'announcing-the-winner-of-the-2027-pen-faulkner-award-for-fiction',
                    'link': 'https://www.penfaulkner.org/2027/04/06/announcing-the-winner-of-the-2027-pen-faulkner-award-for-fiction/',
                }
            ]
        )
        html = _winner_article(2026, 'Small Scale Sinners', 'Mahreen Sohail')
        pages = {
            pf.AWARD_NEWS_REST_URL: rest,
            'https://www.penfaulkner.org/2027/04/06/announcing-the-winner-of-the-2027-pen-faulkner-award-for-fiction/': html,
        }
        results, _tracker = self._lookup(
            pages, 'Small Scale Sinners', 'Mahreen Sohail', utc_year=2027
        )
        self.assertEqual(results, [])
        self.assertIsNone(
            cache.load_cache_entry(
                pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2027', pf.YEAR_CACHE_VERSION
            )
        )

    def test_rest_title_typo_does_not_control_year(self):
        _save_landing(_landing_snapshot())
        rest = json.dumps(
            [
                {
                    'title': {
                        'rendered': 'Announcing the Finalists for the 2028 PEN/Faulkner Award for Fiction'
                    },
                    'slug': 'announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction',
                    'link': 'https://www.penfaulkner.org/2027/03/02/announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction/',
                }
            ]
        )
        html = _finalists_article(2027, _PAIRS_2026, style='prose')
        pages = {
            pf.AWARD_NEWS_REST_URL: rest,
            'https://www.penfaulkner.org/2027/03/02/announcing-the-finalists-for-the-2027-pen-faulkner-award-for-fiction/': html,
        }
        results, tracker = self._lookup(
            pages, 'Dominion', 'Addie E. Citchens', utc_year=2027
        )
        self.assertEqual(results[0].award_year, 2027)
        self.assertIn(pf.AWARD_NEWS_REST_URL, tracker.calls)
        payload = cache.load_cache_entry(
            pf.SOURCE_KEY, pf.YEAR_ENTRY_KIND, '2027', pf.YEAR_CACHE_VERSION
        )
        self.assertEqual(payload['coverage']['state'], 'finalist')
        self.assertNotIn('html', json.dumps(payload))
        self.assertNotIn('wp-json', json.dumps(payload['records']))


if __name__ == '__main__':
    unittest.main()
